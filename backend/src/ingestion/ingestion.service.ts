...
export class IngestionQueueService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    ...
    const connection = redisUrl ? { url: redisUrl } : { host: this.config.get<string>('REDIS_HOST') || 'localhost', port: Number(this.config.get<string>('REDIS_PORT') || 6379) };
  ...
  async enqueue(data: IngestionJobData) {
    ...
    const job = await this.queue.add('generate-course', data, { jobId: data.uploadId });
    return { jobId: job.id, courseId: data.courseId, uploadId: data.uploadId };
  }
  ...
  private async process(job: Job<IngestionJobData>) {
    ...
    try {
      ...
      await this.prisma.$transaction(async (tx) => {
        ...
        for (const mod of generated.modules) {
          ...
          for (const lesson of mod.lessons) {
            ...
            if (lesson.quiz) {
              const quiz = await tx.quiz.create({ data: { lessonId: createdLesson.id, difficulty: lesson.quiz.difficulty, timeLimit: lesson.quiz.timeLimit } });
              if (lesson.quiz.questions?.length) await tx.question.createMany({ data: lesson.quiz.questions.map((q) => ({ quizId: quiz.id, question: q.question, answer: q.answer, options: JSON.stringify(q.options), type: q.type })) });
            }
          }
        }
      });
      ...
      await this.prisma.upload.update({ where: { id: uploadId }, data: { status: 'FAILED' } }).catch(() => undefined);
      throw error;
    }
  }
  ...
  async getJobStatus(jobId: string) {
    ...
    if (!job) return null;
    return { id: job.id, state: await job.getState(), progress: job.progress, failedReason: job.failedReason || null, returnvalue: job.returnvalue || null };
  }
